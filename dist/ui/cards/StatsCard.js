...
export default class StatsCard {
    ...
    drawBackgroundStardust(ctx, width, height) {
        ctx.save();
        const dots = [
    ...
    drawFloatingPetalDrift(ctx, width, height) {
        ctx.save();
        const petals = [
    ...
    drawBotanicalCornerVines(ctx, width, height) {
        ctx.save();
    ...
    async createStatsCard(user, report) {
        ...
        ctx.fillText(`LISTENING PROFILE · ${periodLabels[report.periodLabel] || 'ALL TIME'}`, 122, headerY + 50);
