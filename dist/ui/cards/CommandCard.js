...
export class CommandCard {
    ...
    drawFlowerPetal(ctx, x, y, size, angle, color) {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(angle);
        ctx.beginPath();
    ...
    drawSakuraFlower(ctx, cx, cy, size) {
        ctx.save();
        for (let i = 0; i < 5; i++) {
    ...
    async render(command = {}) {
        ...
        if (isOwnerOnly)
            requirements.push({ text: "👑 Bot Owner Only", color: "#FF5964" });
        if (isUserPrem)
            requirements.push({ text: "💎 User Premium", color: "#FF69B4" });
        if (isGuildPrem)
            requirements.push({ text: "⭐ Server Premium", color: "#FFD166" });
        if (isVoice)
            requirements.push({ text: "🔊 Voice Channel", color: "#74B9FF" });
        if (isSameVoice)
            requirements.push({ text: "👥 Same Channel", color: "#A29BFE" });
        if (isPlayer)
            requirements.push({ text: "🎵 Music Player Active", color: "#00B894" });
        if (isPlaying)
            requirements.push({ text: "▶️ Track Playing", color: "#55EFC4" });
        if (requirements.length === 0)
            requirements.push({ text: "🔓 Public (No Restrictions)", color: "#00CEC9" });
        let reqX = descX + 16;
        let reqYPos = reqY + 36;
        for (const reqObj of requirements) {
            ...
            if (reqYPos > reqY + reqH - 24)
                break; // Clamp inside requirements container height
     ...
     */
    _renderMetricBox(ctx, x, y, w, h, config) {
        ctx.save();
