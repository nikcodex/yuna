...
export default class MusicCard {
    ...
    registerFonts() {
        ...
        try {
            ...
            logger.warn('MusicCard', 'Could not register custom fonts:', e.message);
            fontsRegisteredGlobally = true;
        }
    }
    ...
    drawParticleDropTrail(ctx, x, y, width, height, progress) {
        ctx.save();
    ...
    drawGrowingVineProgressBar(ctx, x, y, width, height, progress, palette) {
        ctx.save();
        const centerY = y + height / 2;
    ...
    drawVinylRecord(ctx, x, y, size, artwork, palette) {
        ctx.save();
    ...
    async createMusicCard(track, position = 0, options = {}) {
        ...
        const sourceStyle = SOURCE_COLORS[rawSource] || SOURCE_COLORS.DEFAULT;
