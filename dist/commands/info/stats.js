...
import { AttachmentBuilder, MessageFlags, ActionRowBuilder, StringSelectMenuBuilder, ContainerBuilder, MediaGalleryBuilder, MediaGalleryItemBuilder, SeparatorBuilder, SeparatorSpacingSize, TextDisplayBuilder, } from "discord.js";
...
class StatsCommand extends Command {
    ...
    async _handleStats(client, context, targetUser, period) {
        ...
        try {
            ...
            // @ts-ignore
            `Failed to generate stats card: ${err.message}`, "Stats Error");
            return this._reply(context, container);
        }
    }
    _buildStatsContainer(user, report, currentPeriod) {
        const container = new ContainerBuilder();
        const gallery = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL("attachment://yuna-stats.png"));
        container.addMediaGalleryComponents(gallery);
        container.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true));
        const stats = report.aggregate || {};
        const streak = stats.current_streak || 0;
        const longestStreak = stats.longest_streak || 0;
        const quickInfo = `🔥 **Current Streak:** ${streak} day${streak !== 1 ? 's' : ''} · ` +
            `**Longest:** ${longestStreak} day${longestStreak !== 1 ? 's' : ''} · ` +
            `**Since:** ${stats.first_listen_date || 'N/A'}`;
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(quickInfo));
        container.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true));
        ...
        // @ts-ignore
        container.addActionRowComponents(new ActionRowBuilder().addComponents(periodMenu));
        return container;
    }
    // @ts-ignore
    _setupPeriodCollector(message, client, targetUser) {
        // @ts-ignore
        const filter = (i) => i.customId === `stats_period_${targetUser.id}`;
        ...
        collector.on("collect", async (interaction) => {
            ...
            try {
                ...
                const errorContainer = buildError(`Failed to generate stats card: ${err.message}`, "Stats Error");
                await interaction.editReply({
