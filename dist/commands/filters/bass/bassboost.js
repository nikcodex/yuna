...
class BassboostFilterCommand extends Command {
    ...
    async _handleFilter(context, pm) {
        try {
            ...
            return this._reply(context, this._createSuccessContainer("Bassboost"));
        }
        catch (error) {
            return this._reply(context, this._createErrorContainer("Could not apply the bassboost filter."));
        }
    }
    _createSuccessContainer(filterName) {
        const content = `**Filter Information**\n\n` +
            ...
            `└─ **${emoji.get("info")} Effect:** Enhanced for bassboost music\n\n` +
