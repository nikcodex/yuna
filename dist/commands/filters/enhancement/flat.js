...
class FlatFilterCommand extends Command {
    constructor() {
        super({
            name: "flat",
    ...
    async _handleFilter(context, pm) {
        try {
            ...
            return this._reply(context, this._createSuccessContainer("Flat"));
        }
        catch (error) {
            return this._reply(context, this._createErrorContainer("Could not apply the flat filter."));
        }
    }
    _createSuccessContainer(filterName) {
        const content = `**Filter Information**\n\n` +
            ...
            `└─ **${emoji.get("info")} Effect:** Enhanced for flat music\n\n` +
