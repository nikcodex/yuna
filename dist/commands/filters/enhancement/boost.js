...
class BoostFilterCommand extends Command {
    constructor() {
        super({
            name: "boost",
    ...
    async _handleFilter(context, pm) {
        try {
            ...
            return this._reply(context, this._createSuccessContainer("Boost"));
        }
        catch (error) {
            return this._reply(context, this._createErrorContainer("Could not apply the boost filter."));
        }
    }
    _createSuccessContainer(filterName) {
        const content = `**Filter Information**\n\n` +
            ...
            `└─ **${emoji.get("info")} Effect:** Enhanced for boost music\n\n` +
