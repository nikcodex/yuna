...
class WarmFilterCommand extends Command {
    constructor() {
        super({
            name: "warm",
    ...
    async _handleFilter(context, pm) {
        try {
            ...
            return this._reply(context, this._createSuccessContainer("Warm"));
        }
        catch (error) {
            return this._reply(context, this._createErrorContainer("Could not apply the warm filter."));
        }
    }
    _createSuccessContainer(filterName) {
        const content = `**Filter Information**\n\n` +
            ...
            `└─ **${emoji.get("info")} Effect:** Enhanced for warm music\n\n` +
