...
class OldschoolFilterCommand extends Command {
    constructor() {
        super({
            name: "oldschool",
    ...
    async _handleFilter(context, pm) {
        try {
            ...
            return this._reply(context, this._createSuccessContainer("Oldschool"));
        }
        catch (error) {
            return this._reply(context, this._createErrorContainer("Could not apply the oldschool filter."));
        }
    }
    _createSuccessContainer(filterName) {
        const content = `**Filter Information**\n\n` +
            ...
            `└─ **${emoji.get("info")} Effect:** Enhanced for oldschool music\n\n` +
