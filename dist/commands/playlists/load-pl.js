...
class LoadPlaylistCommand extends Command {
    ...
    async execute(ctx) {
        ...
        return this._handleLoad(client, context, query, context.user || context.author, positionsStr);
    }
    // @ts-ignore
    ...
    async _handleLoad(client, context, query, user, positionsStr) {
        ...
        try {
            ...
            if (!playlist)
                return this._editReply(loadingMessage, this._createNotFoundContainer(query));
            if (!playlist.tracks || playlist.tracks.length === 0)
                return this._editReply(loadingMessage, this._createEmptyPlaylistContainer(playlist));
            let tracksToProcess;
            let selectionInfo;
            if (positionsStr) {
                const indices = this._parseTrackIndices(positionsStr, playlist.tracks.length);
                if (indices.length === 0) {
            ...
            const queueCheck = this._checkQueueLimit(currentQueueSize, tracksToProcess.length, context.guild.id, user.id);
            ...
            let addedCount = 0;
            let failedCount = 0;
            await this._editReply(loadingMessage, this._createProcessingContainer(playlist.name, 0, finalTracks.length, selectionInfo));
            const BATCH_SIZE = 10;
            for (let i = 0; i < finalTracks.length; i += BATCH_SIZE) {
                const batch = finalTracks.slice(i, i + BATCH_SIZE);
                // @ts-ignore
                const searchResults = await Promise.all(batch.map(async (track) => {
            ...
            return this._editReply(loadingMessage, this._createSuccessContainer(playlist, addedCount, failedCount, finalTracks.length, premiumStatus, queueCheck.limitWarning, wasEmpty && addedCount > 0));
        }
        catch (error) {
    ...
    _createUsageContainer() {
        const content = `**Usage:** \`load-playlist <name_or_id> [positions]\`\n\n` +
    ...
    wasPlaying) {
        ...
        const content = `**Loaded tracks from ${playlist.name}**\n\n` +
    ...
    // @ts-ignore
    _createLoadingContainer(query) { return buildContainer({ title: "Loading", content: `**Loading playlist \`${query}\`...**`, icon: emoji.get("loading") || "ℹ️" }); }
    // @ts-ignore
    _createNotFoundContainer(query) { return buildError(`Could not find a playlist matching \`${query}\`.`, "Playlist Not Found"); }
    _createEmptyPlaylistContainer(playlist) { return buildContainer({ title: "Empty Playlist", content: `The playlist \`${playlist.name}\` has no tracks to load.`, icon: emoji.get("info") || "ℹ️" }); }
    _createErrorContainer(message) { return buildError(message, "Error"); }
    // @ts-ignore
