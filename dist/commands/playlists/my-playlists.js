...
class MyPlaylistsCommand extends Command {
    ...
    async _handleList(user, context, page) {
        ...
        const message = await this._reply(context, this._buildPlaylistsContainer(playlists, page));
        if (message) {
    ...
    _buildPlaylistsContainer(playlists, page) {
        ...
        const pagePlaylists = playlists.slice(startIdx, startIdx + PLAYLISTS_PER_PAGE);
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${emoji.get("folder")} **Your Custom Playlists**`));
    ...
    _setupCollector(message, userId, playlists, initialPage) {
        ...
        collector.on("collect", async (interaction) => {
            if (!interaction.customId.startsWith("mypl_"))
                return;
        ...
        collector.on("end", async () => {
            try {
                ...
                if (error.code !== 10008)
                    logger.error("MyPlaylists", "Failed to clear components on end", error);
            }
        });
    }
    ...
    _createNoPlaylistsContainer() {
        return buildError(`You don't have any custom playlists yet.\n\nUse \`/create-playlist <name>\` to get started!`, "No Playlists Found");
    }
    _formatDuration(ms) {
        if (!ms || ms < 0)
            return "0:00";
