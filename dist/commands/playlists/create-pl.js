...
class CreatePlaylistCommand extends Command {
    ...
    async _handleCreate(user, name, description, context) {
        const loadingMessage = await this._reply(context, this._createLoadingContainer(name));
        try {
            const playlist = db.playlists.createPlaylist(user.id, name, description);
            return this._editReply(loadingMessage, this._createSuccessContainer(playlist));
        }
        catch (error) {
    ...
    _createUsageContainer() {
        const content = `**Missing Playlist Name**\n\n` +
    ...
    _createLoadingContainer(name) {
        const content = `**Setting up your new playlist**\n\n` +
