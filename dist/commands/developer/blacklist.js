...
class BlacklistCommand extends Command {
        ...
        try {
            ...
            if (type === "user") {
                ...
                if (!alreadyBlacklisted) {
                    result = db.users.blacklistUser(id, reason);
                }
            }
            else {
                // @ts-ignore
                ...
                if (!alreadyBlacklisted) {
                    result = db.guilds.blacklistGuild(id, reason);
                }
            }
            if (alreadyBlacklisted) {
            ...
            if (result && result.changes > 0) {
                ...
                return this._sendSuccess(message, "Blacklisted Successfully", `${emoji.get("check")} **Successfully blacklisted ${type}!**\n\n${typeIcon} **Target:** \`${id}\`\n📝 **Reason:** ${reason}`);
            }
            else {
    ...
    async _handleRemove(client, message, args) {
        ...
        try {
            ...
            if (!isUserBlacklisted && !isGuildBlacklisted) {
                return this._sendError(message, "Not Blacklisted", `${emoji.get("cross")} ID \`${id}\` is not blacklisted.`);
            }
            let result;
            let type;
            if (isUserBlacklisted) {
                result = db.users.unblacklistUser(id);
                type = "user";
            }
            else {
                result = db.guilds.unblacklistGuild(id);
                type = "guild";
            }
            if (result && result.changes > 0) {
                const typeIcon = type === "user" ? "👤" : "🏠";
                return this._sendSuccess(message, "Removed from Blacklist", `${emoji.get("check")} **Successfully removed ${type} from blacklist!**\n\n${typeIcon} **Target:** \`${id}\``);
            }
            else {
                return this._sendError(message, "Remove Failed", `${emoji.get("cross")} Failed to remove \`${id}\` from blacklist.`);
            }
        }
        catch (error) {
    ...
    async _handleCheck(client, message, args) {
        ...
        try {
            ...
            const isGuildBlacklisted = db.isGuildBlacklisted(id);
            if (!isUserBlacklisted && !isGuildBlacklisted) {
                return this._sendSuccess(message, "Not Blacklisted", `${emoji.get("check")} **ID \`${id}\` is not blacklisted.**\n\n${emoji.get("info")} This ID is clean and can use the bot normally.`);
            }
            const type = isUserBlacklisted ? "user" : "guild";
            const typeIcon = type === "user" ? "👤" : "🏠";
            return this._sendError(message, "Blacklisted", `${emoji.get("cross")} **${type === "user" ? "User" : "Guild"} is blacklisted!**\n\n${typeIcon} **Target:** \`${id}\`\n${emoji.get("info")} **Type:** ${type.charAt(0).toUpperCase() + type.slice(1)}`);
        }
        catch (error) {
    ...
    _createHelpContainer() {
        ...
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.get("info")} Blacklist Commands Help`));
...
${emoji.get("info")} \`blacklist check @user\``;
    ...
    _sendSuccess(message, title, description) {
        const container = new ContainerBuilder();
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.get("check")} ${title}`));
        container.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
        container.addSectionComponents(new SectionBuilder()
    ...
    _sendError(message, title, description) {
        const container = new ContainerBuilder();
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.get("cross")} ${title}`));
        container.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
        container.addSectionComponents(new SectionBuilder()
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(description))
            .setThumbnailAccessory(new ThumbnailBuilder().setURL(config.assets.defaultThumbnail)));
