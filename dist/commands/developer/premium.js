...
import { ContainerBuilder, TextDisplayBuilder, SectionBuilder, SeparatorBuilder, SeparatorSpacingSize, ThumbnailBuilder, MessageFlags, ActionRowBuilder, ButtonBuilder, ButtonStyle } from "discord.js";
...
class PremiumCommand extends Command {
        ...
        try {
            ...
            if (result && result.changes > 0) {
                const expiryText = expiresAt
                    ...
                    : "Never (Permanent)";
                return this._sendSuccess(message, "Premium Granted", `**Successfully granted ${type} premium!**\n\n**${emoji.get("folder")} Details:**\n├─ **Target:** \`${id}\`\n├─ **Type:** ${type.charAt(0).toUpperCase() + type.slice(1)}\n├─ **Expires:** ${expiryText}\n└─ **Reason:** ${reason}`);
            }
            else {
    ...
    async _handleRevoke(client, message, args) {
        ...
        try {
            ...
            // @ts-ignore
            if (result && result.changes > 0) {
                return this._sendSuccess(message, "Premium Revoked", `**Successfully revoked ${type} premium!**\n\n**${emoji.get("folder")} Details:**\n├─ **Target:** \`${id}\`\n└─ **Type:** ${type.charAt(0).toUpperCase() + type.slice(1)}`);
            }
            else {
                return this._sendError(message, "Revoke Failed", `Failed to revoke premium from ${type} \`${id}\`.\n\nThey may not have active premium or the ID is invalid.`);
            }
        }
        catch (error) {
    ...
    _createHelpContainer() {
        ...
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.get("info")} Premium Management`));
    ...
    _createStatsContainer(stats, userPremiums, guildPremiums, userPage, guildPage) {
        const container = new ContainerBuilder();
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.get("folder")} Premium Statistics`));
        container.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
