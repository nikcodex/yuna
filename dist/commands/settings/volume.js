...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, MessageFlags, PermissionFlagsBits, SectionBuilder, SeparatorBuilder, SeparatorSpacingSize, TextDisplayBuilder, ThumbnailBuilder, } from "discord.js";
...
class SetDefaultVolumeCommand extends Command {
    ...
    async _setDefaultVolume(ctx, volume) {
        try {
            ...
            const container = this._createSuccessContainer("Default Volume Updated", `The default volume for new music players has been set to **${volume}%**.\n\nThis will apply to all new music sessions started in this server.`, volume);
            const replyOptions = {
    ...
    async _showVolumeSettings(ctx) {
        try {
            ...
            container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${emoji.get("info")} **Default Volume Settings**`));
        ...
        collector.on('collect', async (interaction) => {
            if (interaction.customId.startsWith('volume_')) {
                ...
                try {
                    ...
                    const container = this._createSuccessContainer("Default Volume Updated", `The default volume for new music players has been set to **${newVolume}%**.\n\nThis will apply to all new music sessions started in this server.`, newVolume);
    ...
    _createSuccessContainer(title, description, volume) {
        const note = phrases.get("volumeUpdated");
        const volumeIndicator = this._getVolumeIndicator(volume);
        const fullDescription = `${description}\n\n${volumeIndicator}\n\n*${note}*`;
        return buildSuccess(fullDescription, title);
    }
    _getVolumeIndicator(volume) {
        ...
        let volumeLevel = "";
        if (volume <= 25)
            volumeLevel = `${emoji.get("folder")} Low`;
        else if (volume <= 50)
            volumeLevel = `${emoji.get("info")} Medium`;
        else if (volume <= 75)
            volumeLevel = `${emoji.get("check")} High`;
        else
            volumeLevel = `${emoji.get("add")} Maximum`;
        return `**${emoji.get("add")} Volume Level:** ${volumeLevel}\n\`${volumeBar}\` ${volume}%`;
    }
    _createExpiredContainer() {
        return buildError("Interaction Expired. Run the command again to manage volume settings.", "Interaction Expired");
    }
