...
class PresetCommand extends Command {
    ...
    async sendCurrentStyle(context, userId) {
        ...
        const container = new ContainerBuilder()
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.get('info') || 'ℹ️'} Current Player Style`))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addSectionComponents(new SectionBuilder()
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`Your current Now Playing style is set to: **${currentStyle === 'card' ? 'Card Style' : 'Text Style'}**\n\nUse \`.preset <card|text>\` to change it.`))
    ...
    async setStyle(context, userId, style) {
        ...
        const successContainer = buildSuccess(`Your Now Playing style has been successfully changed to **${style === 'card' ? 'Card Style' : 'Text Style'}**.\n\n*${note}*`, 'Style Updated');
