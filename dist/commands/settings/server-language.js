...
class ServerLanguageCommand extends Command {
    ...
    _setupCollector(messageInstance, userId, guildId) {
        ...
        collector.on("collect", async (interaction) => {
            try {
                if (interaction.customId === "server_lang_select") {
                    ...
                    const btnRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("server_lang_confirm").setLabel("Yes, Apply to Server").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId("server_lang_cancel").setLabel("Cancel").setStyle(ButtonStyle.Danger));
