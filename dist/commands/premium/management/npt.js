...
class NoPrefixToggleCommand extends Command {
    ...
    _createUIContainer(username, currentStatus, action = null) {
        ...
        const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
            ...
            .setEmoji(currentStatus ? emoji.get("cross") : emoji.get("check")), new ButtonBuilder()
    ...
    async _handleCommand(ctx, arg) {
        ...
        if (isInteraction) {
            if (ctx.deferred || ctx.replied)
                sent = await ctx.editReply(replyOptions);
            else
                sent = await ctx.reply(replyOptions);
        }
        else {
    ...
    _setupCollector(message, author) {
        ...
        collector.on("collect", async (interaction) => {
            try {
                ...
                if (interaction.customId === "npt_toggle") {
                    ...
                    // @ts-ignore
                    db.users.setNoPrefix(userId, newStatus, null);
                    const updatedContainer = this._createUIContainer(username, newStatus, 
                    // @ts-ignore
        ...
        collector.on("end", async (collected, reason) => {
            ...
            try {
                ...
                if (success) {
                    logger.debug("NoPrefixToggle", `Components disabled successfully. Reason: ${reason}`);
                }
            }
            catch (error) {
        ...
        collector.on("dispose", async (interaction) => {
            logger.debug("NoPrefixToggle", `Interaction disposed: ${interaction.customId}`);
        });
    }
    // @ts-ignore
    ...
    _handleDisableError(error, reason) {
        if (error.code === 10008) {
            logger.debug("NoPrefixToggle", `Message was deleted, cannot disable components. Reason: ${reason}`);
        }
        else if (error.code === 50001) {
            logger.warn("NoPrefixToggle", `Missing permissions to edit message. Reason: ${reason}`);
        }
        else {
            logger.error("NoPrefixToggle", `Error disabling components: ${error.message}. Reason: ${reason}`, error);
        }
    }
