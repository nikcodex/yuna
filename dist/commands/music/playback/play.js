...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, SeparatorBuilder, SeparatorSpacingSize, StringSelectMenuBuilder, UserSelectMenuBuilder, RoleSelectMenuBuilder, ChannelSelectMenuBuilder, MentionableSelectMenuBuilder, ComponentType, } from "discord.js";
...
class PlayCommand extends Command {
    ...
    async execute(ctx) {
        ...
        try {
            ...
            if (!permissions.has(["Connect", "Speak"])) {
                const errorContainer = this._createErrorContainer("I need permission to join and speak in your voice channel.");
                const payload = { components: [errorContainer], flags: MessageFlags.IsComponentsV2 };
                if (context.editReply && (context.deferred || context.replied)) {
                    return await context.editReply(payload);
                }
            ...
            const activePlayer = client.music.getPlayer(context.guild.id) ||
            ...
            if (interaction) {
                await this._updateInteraction(interaction, result, context.guild.id, client, context.user.id);
            }
            else {
                await this._updateMessage(loadingMessage, result, context.guild.id, client, context.author.id);
            }
        }
        catch (error) {
            client.logger?.error("PlayCommand", 
            // @ts-ignore
            `Error in play command: ${error.message}`, error);
    ...
    pm, }) {
        try {
            ...
            client.logger?.error("PlayCommand", 
            // @ts-ignore
            `Error handling play request: ${error.message}`, error);
            return {
                success: false,
    ...
    _createLoadingContainer(query) {
        const content = `**Searching Music Libraries**\n\n` +
            `└─ **Query:-** \`${query}\`\n` +
        ...
        return buildContainer({ image: undefined,
            ...
            content,
            // @ts-ignore
            thumbnail: config.assets.searchIcon || config.assets.defaultTrackArtwork,
    ...
    _createSuccessContainer(result) {
        if (result.type === "playing" || result.type === "queued") {
            ...
            let content = `**Playlist Information**\n\n` +
    ...
    async _updateMessage(message, result, guildId, client, userId) {
        try {
            ...
            client.logger?.error("PlayCommand", 
            // @ts-ignore
            `Error updating message: ${error.message}`, error);
        }
    }
    // @ts-ignore
    async _updateInteraction(interaction, result, guildId, client, userId) {
        try {
            ...
            client.logger?.error("PlayCommand", 
            // @ts-ignore
            `Error updating interaction: ${error.message}`, error);
        }
    }
    // @ts-ignore
    _setupButtonCollector(messageOrInteraction, guildId, client, userId) {
        ...
        collector.on("collect", async (interaction) => {
            ...
            try {
                ...
                if (parts.length < 3) {
                    client.logger?.warn("PlayCommand", `Invalid customId format: ${interaction.customId}`);
                    return;
                }
                parts.pop();
                ...
                if (isNaN(trackIndex)) {
                    client.logger?.warn("PlayCommand", `Invalid track index: ${trackIndexStr}`);
                    return;
                }
                ...
                switch (action) {
                    case "play_now":
                        ...
                        newContainer = this._createActionResultContainer("Track Updated", actionMessage);
                    ...
                    case "play_next":
                        ...
                        newContainer = this._createActionResultContainer("Queue Updated", actionMessage);
                        actionCompleted = true;
                        break;
                    case "remove_track":
                        // @ts-ignore
                        ...
                        newContainer = this._createActionResultContainer("Track Removed", actionMessage);
                        actionCompleted = true;
                        break;
                    default:
                ...
                if (actionCompleted && newContainer) {
                    ...
                    client.logger?.debug("PlayCommand", `Action completed: ${action} for track ${trackIndex}`);
                }
            }
            catch (error) {
                client.logger?.error("PlayCommand", 
                // @ts-ignore
                `Error in button collector: ${error.message}`, error);
                try {
                    if (!actionCompleted) {
        ...
        collector.on("end", async (collected, reason) => {
            ...
            try {
                ...
                if (!currentMessage?.components?.length) {
                    client.logger?.debug("PlayCommand", "No message or components found for disabling");
                    return;
                }
                const success = await this._disableAllComponents(currentMessage, client);
                if (success) {
                    client.logger?.debug("PlayCommand", `Components disabled successfully. Reason: ${reason}`);
                }
            }
            catch (error) {
        ...
        collector.on("dispose", async (interaction) => {
            client.logger?.debug("PlayCommand", `Interaction disposed: ${interaction.customId}`);
        });
    }
    // @ts-ignore
    ...
    async _disableComponents(currentMessage, client) {
        try {
            const containerComponent = currentMessage.components.find((c) => c.type === ComponentType.Container);
    ...
    _handleDisableError(error, client, reason) {
        ...
        if (!ignoredErrors.includes(error.code)) {
            client.logger?.error("PlayCommand", `Error disabling components after ${reason}: ${error.message}`, {
                code: error.code,
                reason,
                stack: error.stack,
            });
            ...
            client.logger?.debug("PlayCommand", `Ignored error ${error.code} when disabling components: ${error.message}`);
        }
    }
    // @ts-ignore
    ...
    _normalizeSource(source) {
        ...
        // @ts-ignore
        return sourceMap[source?.toLowerCase()] || musicSource.PRIMARY_PREFIX.replace(':', '');
    }
    // @ts-ignore
    _isUrl(string) {
        try {
