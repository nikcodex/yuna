...
class PreviousCommand extends Command {
    ...
    async _handlePrevious(context, pm) {
        ...
        if (!success) {
            return this._reply(context, this._createErrorContainer(phrases.get("errorGeneric")));
        }
        ...
        const content = `**Playing Previous Track**\n\n` +
            ...
            `└─ **${emoji.get("info") || "ℹ️"} Duration:** ${this._formatDuration(previousTrack.info.duration)}`;
