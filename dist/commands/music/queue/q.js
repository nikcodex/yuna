...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, StringSelectMenuBuilder, ComponentType, SeparatorBuilder, SeparatorSpacingSize, TextDisplayBuilder, SectionBuilder, ThumbnailBuilder } from "discord.js";
...
class QueueCommand extends Command {
    ...
    _buildQueueContainer(player, page, guildId, userId) {
        ...
        const totalDuration = tracks.reduce((acc, t) => acc + (t.info?.duration || 0), current.info?.duration || 0);
        ...
        if (tracks.length > 0) {
            ...
            paginatedTracks.forEach((track, index) => {
                ...
                container.addSectionComponents(new SectionBuilder()
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`\`${trueIndex + 1}.\` [**${track.info.title}**](${track.info.uri})\n└ *${track.info.author} • \`${formatDuration(track.info.duration)}\` • by <@${track.requester?.id || userId}>*`))
            ...
            container.addActionRowComponents(this._createPaginationRow(currentPage, maxPages, guildId));
            // Action Row 2: Queue Management Controls
            container.addActionRowComponents(this._createQuickControlsRow(guildId));
        }
        else {
            container.addTextDisplayComponents(new TextDisplayBuilder().setContent("*(No more tracks in queue. Add more songs using `/play`!)*"));
        }
        return container;
    }
    // @ts-ignore
    _createJumpMenu(tracksOnPage, startIndex, guildId) {
        if (!tracksOnPage || tracksOnPage.length === 0)
            return null;
    ...
    // @ts-ignore
    _createPaginationRow(currentPage, maxPages, guildId) {
    ...
    // @ts-ignore
    _createQuickControlsRow(guildId) {
        return new ActionRowBuilder().addComponents(new ButtonBuilder()
    ...
    // @ts-ignore
    async _setupCollector(message, client, guildId, initialPage, userId) {
        const filter = (i) => i.customId.startsWith("q_") && i.customId.endsWith(guildId);
        ...
        collector.on("collect", async (interaction) => {
            ...
            if (action === "page") {
                ...
                try {
                    const playlist = db.playlists.create(interaction.user.id, plName, `Saved from live queue on ${new Date().toLocaleString()}`);
                    for (const t of allTracks) {
                        db.playlists.addTrack(playlist.id, t.info || t);
                    }
        ...
        return components.map(c => {
            if (c.type === ComponentType.ActionRow) {
                return {
                    ...c.toJSON(),
                    // @ts-ignore
                    components: c.components.map(sub => ({ ...sub.toJSON(), disabled: true }))
                };
            }
