...
class RadioCommand extends Command {
    ...
    // @ts-ignore
    async _sendRadioMenu(context, client) {
        ...
        if (msg) {
            ...
            // @ts-ignore
            const filter = (i) => i.customId === "radio_select_menu" && i.user.id === userId;
    ...
    // @ts-ignore
    async _playRadioStation(context, client, station, existingMsg = null) {
        try {
            const button = new ButtonBuilder()
                .setLabel("Support")
                .setURL(config.links?.supportServer || "https://discord.gg/XYwwyDKhec")
