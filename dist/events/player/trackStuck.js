    ...
    async execute(player, track, payload, musicManager, client) {
        try {
                ...
                try {
                    ...
                    if (message) {
                        const stuckContainer = new ContainerBuilder()
                            ...
                            .addSectionComponents(new SectionBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`Attempting to recover: **${track?.info?.title || "Track"}**`)));
                        await message.edit({
            ...
            const warnContainer = new ContainerBuilder()
                ...
                .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
                .addSectionComponents(new SectionBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent("Audio playback is experiencing issues. Attempting to recover automatically...")));
