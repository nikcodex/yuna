...
export class GuildRepo {
    ...
    ensureGuild(guildId) {
        if (!guildId)
            throw new Error('A valid guildId must be provided');
    ...
    set247Mode(guildId, enabled, voiceChannelId = null, textChannelId = null) {
        this.ensureGuild(guildId);
        const guild = this.getGuild(guildId);
        this._updateGuild.run(guild.prefixes, guild.default_volume, enabled ? 1 : 0, enabled ? 1 : 0, enabled ? voiceChannelId : null, enabled ? textChannelId : null, guildId);
    }
    /**
     ...
     */
    saveActiveSession(guildId, voiceChannelId, textChannelId, currentTrack = null, queueTracks = [], position = 0) {
