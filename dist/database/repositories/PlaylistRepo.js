...
export class PlaylistRepo {
    ...
    updatePlaylist(playlistId, userId, updates) {
        ...
        if (updates.name !== undefined) {
            if (!updates.name || updates.name.length > PLAYLIST_NAME_MAX_LENGTH)
                throw new Error('Invalid playlist name');
            // @ts-ignore
        ...
        if (updates.description !== undefined) {
            if (updates.description && updates.description.length > PLAYLIST_DESCRIPTION_MAX_LENGTH)
                throw new Error('Description too long');
    ...
    addTrackToPlaylist(playlistId, userId, trackInfo) {
        ...
        // @ts-ignore
        if (tracks.some(t => t.identifier === identifier))
    ...
    cleanupPlaylists(userId) {
        ...
        this.db.transaction(() => {
            for (const playlist of playlists) {
                try {
                    // @ts-ignore
                    const tracks = playlist.tracks.filter(track => track && track.identifier && track.title);
                    // @ts-ignore
                    const totalDuration = tracks.reduce((sum, track) => sum + (track.duration || 0), 0);
