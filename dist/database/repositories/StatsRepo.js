...
export class StatsRepo {
     ...
     */
    logTrackPlay(userId, trackInfo) {
        try {
            this.db.transaction(() => {
                this._ensureStats.run(userId);
                this._logPlay.run(userId, trackInfo.title || 'Unknown', trackInfo.author || 'Unknown', trackInfo.identifier || null, trackInfo.uri || null, trackInfo.sourceName || null, trackInfo.artworkUrl || null, trackInfo.duration || trackInfo.length || 0);
     ...
     */
    // @ts-ignore
    getTopArtists(userId, since = '1970-01-01', limit = 5) {
     ...
     */
    // @ts-ignore
    getTopTracks(userId, since = '1970-01-01', limit = 5) {
     ...
     */
    // @ts-ignore
    getTopSources(userId, since = '1970-01-01', limit = 5) {
     ...
     */
    // @ts-ignore
    getRecentPlays(userId, limit = 10) {
     ...
     */
    // @ts-ignore
    getFullReport(userId, period = 'all') {
