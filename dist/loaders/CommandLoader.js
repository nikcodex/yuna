    ...
    async loadCommands(dirPath = '../commands') {
        ...
        try {
            ...
            if (this.failedCommands.length === 0) {
                ...
                console.log(`${logger.formatTime()} ${pink('🌸 [Commands]')} ${mint('✔ All ' + cmdCount + ' commands loaded successfully')} ${gray(`(${catCount} categories · ${slashCount} slash)`)}`);
            }
            else {
                console.log(`${logger.formatTime()} ${pink('🌸 [Commands]')} ${red(`✖ Failed to load ${this.failedCommands.length} command(s):`)}`);
                for (const failure of this.failedCommands) {
                    console.log(`   ${red('└─')} ${white(failure.file)}: ${red(failure.error)}`);
                }
                console.log(`${logger.formatTime()} ${pink('🌸 [Commands]')} ${mint(`✔ ${this.client.commands.size} command(s) loaded successfully`)}`);
            }
        }
        catch (error) {
    ...
    async _loadCommandFile(filePath, category) {
        ...
        try {
            ...
            if (slashEnabled && slashData) {
                const rawName = typeof slashData.name === 'function' ? slashData.name : slashData.name;
            ...
            this.failedCommands.push({ file: relName, error: error.message || error });
        }
    }
