...
export class Scheduler {
     ...
     */
    every(name, interval = {}, taskFunc = null) {
        if (taskFunc)
            this.register(name, taskFunc);
        const totalSeconds = (interval.seconds || 0) +
     ...
     * @param {Function} [taskFunc]
     */
    cron(name, pattern, taskFunc = null) {
        if (taskFunc)
            this.register(name, taskFunc);
