...
import { checkPermissions, checkBotPermissions, isPremium, isOwner, isBlacklisted } from '../core/AccessControl.js';
export class Middleware {
     ...
     */
    static checkCooldown(userId, command) {
        if (!command.cooldown)
            return 0;
        ...
        if (this.cooldowns.has(key)) {
            const expirationTime = this.cooldowns.get(key) + cooldownMs;
    ...
    static setCooldown(userId, command) {
        if (!command.cooldown)
            return;
        const key = `${userId}-${command.name}`;
