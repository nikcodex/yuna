...
function dissectURL(url) {
    ...
    if (replacedURL.startsWith('discordjs')) {
        const splitterRegex = /(?:discordjs|discord-akairo)\/(?:docs\/main\/)?(?<repo>[^/]*)\/(?:docs\/)?(?<branch>[^/]*)/;
...
export class Doc extends DocBase {
    ...
    get(...terms) {
        ...
        if (!elem || !lowerCasedTerms.length)
            return elem || null;
    ...
    search(query, { excludePrivateElements, maxResults = 10 } = {}) {
        ...
        while (result.length > 0 && filtered.length < maxResults) {
            ...
            if (excludePrivateElements && element?.access === 'private')
                continue;
            if (element)
    ...
    toJSON() {
        const json = { classes: [], typedefs: [], interfaces: [] };
