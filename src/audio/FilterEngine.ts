...
export class FilterEngine {
  ...
  static async apply(player: Player, presetName: string) {
    if (!player || !player.filterManager) return false;
    ...
    if (preset.bands) {
      ...
      (player.filterManager as any).data.timescale = preset.timescale;
      await player.filterManager.applyPlayerFilters();
    }
    return true;
  }

  static async reset(player: Player) {
    if (!player || !player.filterManager) return false;
  ...
  static getPresetInfo(name: string) {
